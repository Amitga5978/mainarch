...
#[command(
    ...
    version,
    about = "CPU-only reference MoE model API readiness selftest"
)]
struct Args {
    /// Decoder layers to emit in the reference graph.
    #[arg(long, default_value_t = 2)]
    layers: usize,
    /// Vocabulary rows to declare for the compact LM head.
    #[arg(long, default_value_t = 1024)]
    vocab: usize,
    /// Maximum context rows to declare for the compact paged KV cache.
    #[arg(long, default_value_t = 128)]
    max_context: usize,
    /// Emit only the runtime launch request receipt.
    #[arg(long)]
    runtime_launch_request_receipt: bool,
    /// Emit only the runtime submission gate receipt.
    #[arg(long)]
    runtime_submission_gate_receipt: bool,
    /// Emit only the resolved runtime submission gate receipt.
    #[arg(long)]
    runtime_resolved_submission_gate_receipt: bool,
    /// Emit only the resolved runtime submission prerequisite plan receipt.
    #[arg(long)]
    runtime_resolved_submission_prerequisite_plan_receipt: bool,
    /// Emit only the resolved runtime submission blocker report receipt.
    #[arg(long)]
    runtime_resolved_submission_blocker_report_receipt: bool,
    /// Emit only the runtime submission blocker report receipt.
    #[arg(long)]
    runtime_submission_blocker_report_receipt: bool,
    /// Emit only the runtime submission prerequisite plan receipt.
    #[arg(long)]
    runtime_submission_prerequisite_plan_receipt: bool,
    /// Emit only the static plugin handoff receipt.
    #[arg(long)]
    static_handoff_receipt: bool,
}
...
fn main() -> Result<()> {
    ...
    if args.static_handoff_receipt {
        let static_handoff = plugin_inspection.synthetic_cpu_static_handoff_receipt("selftest")?;
    ...
    let dispatch_scalar_args = readiness
        ...
        .sum::<usize>();
    let slot_bindings = readiness.slots.metadata_binding_template("selftest")?;
    ...
    let launch_kernel_argument_abi = readiness
    ...
    let launch_kernel_argument_abi_size_receipt = readiness
    ...
    let launch_kernel_argument_abi_gaps = readiness
    ...
    let launch_kernel_argument_abi_capacity_requests = readiness
    ...
    let launch_kernel_argument_abi_semantics = readiness
    ...
    let launch_kernel_argument_abi_semantic_gaps = readiness
    ...
    let launch_kernel_argument_abi_semantic_projection = readiness
    ...
    let launch_kernel_argument_abi_semantic_projection_gaps = readiness
    ...
    let launch_execution = readiness.runtime_launch_execution_readiness_report(
        &slot_bindings,
        &code_object,
        DEFAULT_RUNTIME_LAUNCH_WINDOW_DISPATCHES,
        &device_pointer_validation,
    ...
    let launch_execution_requests = readiness.runtime_launch_execution_request_plan(
        &slot_bindings,
        &code_object,
        DEFAULT_RUNTIME_LAUNCH_WINDOW_DISPATCHES,
        &device_pointer_validation,
    ...
    if args.runtime_launch_request_receipt {
        print!("{}", launch_execution_requests.receipt_text());
        return Ok(());
    }
    let launch_submission_gate = readiness.runtime_launch_submission_gate(
        &slot_bindings,
        &code_object,
        DEFAULT_RUNTIME_LAUNCH_WINDOW_DISPATCHES,
        &device_pointer_validation,
    ...
    if args.runtime_submission_gate_receipt {
        print!("{}", launch_submission_gate.receipt_text());
        return Ok(());
    }
    let launch_submission_blockers = readiness.runtime_launch_submission_blocker_report(
        &slot_bindings,
        &code_object,
        DEFAULT_RUNTIME_LAUNCH_WINDOW_DISPATCHES,
        &device_pointer_validation,
    ...
    if args.runtime_submission_blocker_report_receipt {
        print!("{}", launch_submission_blockers.receipt_text());
        return Ok(());
    }
    let launch_submission_prerequisites = readiness.runtime_launch_submission_prerequisite_plan(
        &slot_bindings,
        &code_object,
        DEFAULT_RUNTIME_LAUNCH_WINDOW_DISPATCHES,
        &device_pointer_validation,
    ...
    if args.runtime_submission_prerequisite_plan_receipt {
    ...
    let launch_validation_application_plan = launch_execution_requests
        .live_aql_proof_validation_application_plan(&live_aql_proof_validations)?;
    launch_validation_application_plan.assert_non_submitting_boundary()?;
    ...
    assert!(launch_resolved_submission_prerequisites
        ...
        .is_empty());
    if args.runtime_resolved_submission_prerequisite_plan_receipt {
        print!(
            "{}",
    ...
    if args.runtime_resolved_submission_blocker_report_receipt {
        print!(
            "{}",
    ...
    if args.runtime_resolved_submission_gate_receipt {
    ...
    let checkpoint_payloads = readiness.synthetic_cpu_runtime_checkpoint_payload_binding_plan(
        ...
        "selftest.synthetic.safetensors",
        &[1002, 1001],
        DEFAULT_RUNTIME_SYNTHETIC_DEVICE_POINTER_BASE,
        DEFAULT_RUNTIME_DEVICE_POINTER_ALIGNMENT,
    )?;
    ...
    println!("mainarch-model-api-selftest - CPU-only reference MoE model API readiness");
    println!(
    ...
    println!(
        "  model-api ok: model={} tensors={} ops={} stages={} staged_ops={} unstaged_ops={}",
        graph.name,
    ...
    println!(
        "  storage: total_bytes={} weight_bytes={} kv_cache_bytes={} activation_bytes={}",
        readiness.storage.footprint.total_bytes,
        readiness.storage.footprint.weight_bytes,
        readiness.storage.footprint.kv_cache_bytes,
        readiness.storage.footprint.activation_bytes
    );
    println!(
    ...
    println!(
        "  slots: tensors={} op_rows={} inputs={} outputs={} checkpoint_weights={} persistent_state={} scratch={}",
        ...
        readiness.slots.persistent_state_slots.len(),
